...
The contract follows the current ItsyGitsy source parser, with two deliberate refinements: adjacent line-comment markers are separate annotations, and blocks with trailing executable text are ignored to avoid concealing code. Shared consumers should use these tests when adopting this contract.
